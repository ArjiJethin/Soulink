  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errMsg =
          data?.error || data?.message || response.statusText || "Failed to save";
        ...
        <main className="q-card-body">
          <div className="q-header">
            <div>
              ...
              <p className="q-subtitle">Answer the following questions to reflect on your day</p>
            </div>

            <header className="q-card-header">
              <div className="q-header-actions">
                <button className="q-back-btn" onClick={handleBack} aria-label="Back">
                  <i className="fa-regular fa-house color-blue"></i>
                </button>
                <div className="q-avatar-circle" title="Your profile">
                  <img src={userAvatarUrl} alt="Avatar" style={{ width: "100%", height: "100%" }} />
                </div>
              </div>
            </header>
          </div>
            ...
            <div className="q-questions-list">
              {questions.map((q, index) => (
                <div key={index} className="q-question-block">
                  ...
                  <div className="q-options" role="radiogroup" aria-label={`Question ${index + 1}`}>
                    {options.map((opt, i) => (
                      <label key={i} className="q-option" aria-checked={answers[index] === opt}>
                        <input
                          type="radio"
                          name={`q-${index}`}
                          value={opt}
