...
export default function Dashboard() {
  ...
  const fetchAISuggestions = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/dashboard/suggestions?user_id=default_user`);
      if (response.ok) {
        const data = await response.json();
        setAiSuggestions(data.suggestions.map((suggestion, index) => ({
          id: index + 1,
  ...
  const fetchWellnessData = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/dashboard/wellness?user_id=default_user`);
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchTodaysMood = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/dashboard/mood?user_id=default_user`);
      if (response.ok) {
        const data = await response.json();
        setTodaysMood({
          mood: data.mood,
      ...
      <div className="dashboard-content">
          ...
          <div className="character-avatar">
            <div className="avatar-display">
              <div className="avatar-image-large">
                <img 
                  src={userPreferences.selectedAvatar?.image || defaultAvatar} 
                  alt={userPreferences.selectedAvatar?.name || "Avatar"} 
                  className="avatar-img"
                />
              </div>
              <div className="avatar-info-display">
                <h2 className="username-display">Hello, {userPreferences.username}!</h2>
                <p className="avatar-name-display">{userPreferences.selectedAvatar?.name || "Friendly"} Avatar</p>
                <p className="avatar-description-display">
                  {userPreferences.selectedAvatar?.description || "Cheerful and optimistic"}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="dashboard-grid">
          ...
          >
            ...
            <p className="card-description">Write down your thoughts and feelings</p>
          </div>
          ...
          <div className="dashboard-card mood-card">
            ...
            <div className="mood-content">
              <div className="mood-emoji">{getMoodEmoji(todaysMood.mood)}</div>
              <p className="mood-message">{todaysMood.message}</p>
              <p className="mood-encouragement">✨ {todaysMood.encouragement}</p>
            </div>
          </div>
          ...
          <div className="dashboard-card wellness-card">
            ...
            <div className="wellness-content">
              <div className="wellness-score">
                ...
                <span className="score-number">{wellnessData.score}</span>
              </div>
              <div className="wellness-stats">
                <div className="stat-item">
                  ...
                  <div className="trend-container">
                    ...
                    <span className="stat-value">{wellnessData.trend}%</span>
                  </div>
                </div>
                <div className="stat-item">
            ...
            <div className="characters-content">
              {userPreferences.characters && userPreferences.characters.length > 0 ? (
                <div className="characters-grid">
                  {userPreferences.characters.map((character, index) => (
                    <div key={index} className="character-item">
                      ...
                      <div className="character-info">
                        ...
                        <span className="character-relation">{character.relation}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="characters-placeholder">
                  <div className="character-item">
                    ...
                    <div className="character-info">
                      <span className="character-name">Mom</span>
                      <span className="character-relation">Family</span>
                    </div>
                  </div>
                  <div className="character-item">
                    <div className="character-avatar-small">👨</div>
                    <div className="character-info">
                      <span className="character-name">Best Friend</span>
                      <span className="character-relation">Friend</span>
                    </div>
                  </div>
                  <div className="preferences-note">
                    <small>💡 Add characters in preferences</small>
                  </div>
                </div>
              )}
            </div>
